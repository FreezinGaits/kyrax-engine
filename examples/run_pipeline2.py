...
def main():
    ...
    dry_flag = os.environ.get("KYRAX_OS_DRY_RUN", "true")
    dry_run = str(dry_flag).strip().lower() in ("1", "true", "yes")

    os_skill = OSSkill(dry_run=dry_run)
    ...
    try:
        while True:
            try:
                ...
                # ============================================================
                # 🔹 REGEX OS FAST-PATH (OFFLINE, NO LLM)
                # ============================================================

                # ---- volume shorthand: "volume 30" ----
                # m = re.fullmatch(r'\s*volume\s+(\d{1,3})\s*', raw, re.I)
                # if m:
                ...
                #         source="regex_volume"
                #     )
                #     print("Detected volume command (regex):", cmd)
                #     print("Result:", dispatcher.execute(cmd))
                #     continue


                # ---- mute ----
                # if raw.lower() in ("mute", "mute volume"):
                #     cmd = Command(
                #         intent="mute",
                #         domain="os",
                #         entities={},
                ...
                #         source="regex_os"
                #     )
                #     print("Detected mute command (regex):", cmd)
                #     print("Result:", dispatcher.execute(cmd))
                #     continue


                # ---- unmute ----
                # if raw.lower() in ("unmute", "unmute volume"):
                #     cmd = Command(
                #         intent="unmute",
                #         domain="os",
                #         entities={},
                ...
                #         source="regex_os"
                #     )
                #     print("Detected unmute command (regex):", cmd)
                #     print("Result:", dispatcher.execute(cmd))
                #     continue


                # ---- open app: "open chrome" ----
                # m = re.fullmatch(r'\s*open\s+(.+)\s*', raw, re.I)
                # if m:
                #     app = m.group(1).strip()
                #     cmd = Command(
                #         intent="open_app",
                ...
                #         source="regex_os"
                #     )
                #     print("Detected open-app command (regex):", cmd)
                #     print("Result:", dispatcher.execute(cmd))
                #     continue

                # ============================================================
                # ============================================================
                # 🔹 REGEX MULTI-DOMAIN CLAUSE PARSER (COLLECT ONLY)
                # ============================================================
                for clause in clauses:

                    # ---- volume ----
                    m = re.search(r'\bvolume\b.*?(\d{1,3})', clause, re.I)
                    if m:
                        level = max(0, min(100, int(m.group(1))))
                    ...
                    # ---- unmute ----
                    if clause.lower() in ("unmute", "unmute volume"):
                        detected_commands.append(
                            Command(
                                intent="unmute",
                                domain="os",
                    ...
                    # ---- open app ----
                    m = re.fullmatch(r'\s*open\s+(.+)\s*', clause, re.I)
                    if m:
                        app = m.group(1).strip()
                        detected_commands.append(
                            Command(
                    ...
                    # ---- send messages (reuse your existing extractor) ----
                    send_cmds = extract_send_commands(clause)
                    for step in send_cmds:
                        detected_commands.append(
                            Command(
                                intent="send_message",
                                domain="application",
                ...
                if detected_commands:
                    print(f"Detected {len(detected_commands)} command(s). Executing sequentially.")
                    for cmd in detected_commands:
                        ...
                        if guard_res.get("status") == "executed":
                            print("Result:", guard_res["result"])
                            if getattr(guard_res["result"], "success", False):
                                ctx_logger.update_from_command(cmd)
                        else:
                            print("Guard result:", guard_res)
                    continue  # 🔴 CRITICAL: prevents falling into LLM path
            ...
            # # ---------- OS VOLUME FAST-PATH (NO LLM REQUIRED) ----------
            # m = re.search(r'\b(set|change|adjust)\s+volume\s+(to\s+)?(\d{1,3})', raw.lower())
            # if m:
            #     level = int(m.group(3))
            ...
            looks_like_os = bool(
                re.search(r'\b(volume|mute|unmute|open|close|shutdown|restart|sleep)\b', raw.lower())
            )
